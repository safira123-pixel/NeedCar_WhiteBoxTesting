...
class TransactionController extends Controller
{
    ...
    public function source($status){
        ...
        ->filter(function ($query) {
            ...
            ->addIndexColumn()
            ->addColumn('action', '.transaction.index-action')
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('transaction.index')->with('success-message','Data telah dirubah');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->back()->with('error-message',$e->getMessage());
        }
    ...
    public function print($id){
        $data = $this->transaction->find($id);
        // return view('.transaction.cetak',compact(['data']));
        $pdf = PDF::loadView('transaction.cetak',compact(['data']));
